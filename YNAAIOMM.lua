...
local function spawn_t20()
    handle_vehicle_spawn("T20", "t20")
end

local function max_upgrades()
    local veh = self.get_veh()
    if veh ~= 0 then
...
local function custom_license_plate_ethereal()
    local veh = self.get_veh()
    if veh ~= 0 then
...
local function give_all_weapons()
    local ped = self.get_ped()
    local weapons = {
        "WEAPON_BAT", "WEAPON_KNIFE", "WEAPON_HAMMER", "WEAPON_CROWBAR", "WEAPON_GOLFCLUB", "WEAPON_BOTTLE",
        "WEAPON_DAGGER", "WEAPON_HATCHET", "WEAPON_KNUCKLE", "WEAPON_MACHETE", "WEAPON_FLASHLIGHT", "WEAPON_SWITCHBLADE",
        ...
        "WEAPON_HOMINGLAUNCHER", "WEAPON_COMPACTLAUNCHER", "WEAPON_RAYMINIGUN", "WEAPON_EMPLAUNCHER",
        "WEAPON_GRENADE", "WEAPON_BZGAS", "WEAPON_MOLOTOV", "WEAPON_STICKYBOMB", "WEAPON_PROXMINE", "WEAPON_SNOWBALL",
        "WEAPON_PIPEBOMB", "WEAPON_FLARE",
...
local function max_all_weapons()
    ...
    local generic_components = {
        ...
        "COMPONENT_AT_AR_SUPP", "COMPONENT_AT_AR_SUPP_02", "COMPONENT_AT_PI_SUPP", "COMPONENT_AT_PI_SUPP_02",
        ...
        "COMPONENT_AT_SCOPE_MEDIUM", "COMPONENT_AT_SCOPE_LARGE", "COMPONENT_AT_SCOPE_MAX", "COMPONENT_AT_SCOPE_NV",
        "COMPONENT_AT_SCOPE_THERMAL",
        "COMPONENT_AT_PI_COMP", "COMPONENT_AT_PI_COMP_02", "COMPONENT_AT_PI_COMP_03",
        "COMPONENT_AT_MUZZLE_01", "COMPONENT_AT_MUZZLE_02", "COMPONENT_AT_MUZZLE_03", "COMPONENT_AT_MUZZLE_04",
        "COMPONENT_AT_MUZZLE_05", "COMPONENT_AT_MUZZLE_06", "COMPONENT_AT_MUZZLE_07", "COMPONENT_AT_MUZZLE_08",
...
local function delete_nearby_vehicles()
    script.run_in_fiber(function()
        local count = 0
        local my_veh = self.get_veh()
        for _, v in ipairs(entities.get_all_vehicles()) do
...
local function fetch_random_joke_http()
...
gui.register_menu("YNAAIOMM", function()
    ...
    gui.add_option("👯 Clone Ped", "Spawn a clone of yourself", clone_ped)
    ...
    gui.add_option("✨ Suicide", "Take the easy way out", suicide)
    ...
    gui.add_option("✨ Stop Animation", "Clear current tasks", stop_animation)
    gui.add_option("✨ Pole Dance", "Dancing on a pole", pole_dance)
    gui.add_option("✨ Push Ups", "Workout animation", push_ups)
    gui.add_option("✨ Drink Beer", "Drinking animation", drink_beer)
    gui.add_option("✨ Cheering", "Cheer animation", cheering)
    gui.add_option("✨ Cowering", "Cower animation", cowering)

    gui.set_submenu_context(animate_menu)
    local scenarios_menu = gui.add_submenu("Scenarios", "Play world scenarios")
    gui.set_submenu_context(scenarios_menu)
    gui.add_option("✨ Stop Scenario", "Clear current tasks", stop_scenario)
    gui.add_option("✨ Drink Coffee", "Drink a cup of coffee", drink_coffee)
    gui.add_option("✨ Muscle Flex", "Flex your muscles", muscle_flex)
    gui.add_option("✨ Paparazzi", "Take pictures", paparazzi)
    gui.add_option("✨ Guard Stand", "Stand like a guard", guard_stand)
    gui.add_option("✨ Jogging", "Go for a jog", jogging)
    gui.add_option("✨ Fishing", "Stand and fish", fishing)
    gui.add_option("✨ Binoculars", "Look through binoculars", binoculars)
    ...
    gui.add_option("✨ pipistrello", "Spawn pipistrello",
        function() handle_vehicle_spawn("pipistrello", "pipistrello") end)
    gui.set_submenu_context(super_menu)
    ...
    gui.add_option("✨ wastelander", "Spawn wastelander",
        function() handle_vehicle_spawn("wastelander", "wastelander") end)
    ...
    gui.add_option("✨ armytrailer", "Spawn armytrailer",
        function() handle_vehicle_spawn("armytrailer", "armytrailer") end)
    gui.add_option("✨ armytrailer2", "Spawn armytrailer2",
        function() handle_vehicle_spawn("armytrailer2", "armytrailer2") end)
    gui.add_option("✨ baletrailer", "Spawn baletrailer",
        function() handle_vehicle_spawn("baletrailer", "baletrailer") end)
    gui.add_option("✨ boattrailer", "Spawn boattrailer",
        function() handle_vehicle_spawn("boattrailer", "boattrailer") end)
    ...
    gui.add_option("✨ docktrailer", "Spawn docktrailer",
        function() handle_vehicle_spawn("docktrailer", "docktrailer") end)
    ...
    gui.add_option("✨ proptrailer", "Spawn proptrailer",
        function() handle_vehicle_spawn("proptrailer", "proptrailer") end)
    gui.add_option("✨ raketrailer", "Spawn raketrailer",
        function() handle_vehicle_spawn("raketrailer", "raketrailer") end)
    ...
    gui.add_option("✨ trailerlogs", "Spawn trailerlogs",
        function() handle_vehicle_spawn("trailerlogs", "trailerlogs") end)
    ...
    gui.add_option("✨ utillitruck", "Spawn utillitruck",
        function() handle_vehicle_spawn("utillitruck", "utillitruck") end)
    ...
    gui.add_option("✨ polbuffalo6", "Spawn polbuffalo6",
        function() handle_vehicle_spawn("polbuffalo6", "polbuffalo6") end)
    gui.set_submenu_context(emergency_menu)
    ...
    gui.add_option("✨ polcaracara", "Spawn polcaracara",
        function() handle_vehicle_spawn("polcaracara", "polcaracara") end)
    gui.add_option("✨ polcoquette4", "Spawn polcoquette4",
        function() handle_vehicle_spawn("polcoquette4", "polcoquette4") end)
    gui.add_option("✨ polfaction2", "Spawn polfaction2",
        function() handle_vehicle_spawn("polfaction2", "polfaction2") end)
    gui.add_option("✨ polterminus", "Spawn polterminus",
        function() handle_vehicle_spawn("polterminus", "polterminus") end)
    gui.set_submenu_context(emergency_menu)
    ...
    gui.add_option("✨ polimpaler5", "Spawn polimpaler5",
        function() handle_vehicle_spawn("polimpaler5", "polimpaler5") end)
    gui.add_option("✨ polimpaler6", "Spawn polimpaler6",
        function() handle_vehicle_spawn("polimpaler6", "polimpaler6") end)
    gui.set_submenu_context(emergency_menu)
    ...
    gui.add_option("✨ polgauntlet", "Spawn polgauntlet",
        function() handle_vehicle_spawn("polgauntlet", "polgauntlet") end)
    ...
    gui.add_option("✨ dominator10", "Spawn dominator10",
        function() handle_vehicle_spawn("dominator10", "dominator10") end)
    gui.set_submenu_context(muscle_menu)
    ...
    local function apply_comp(suffix, use_generic, generic_hash_string)
        ...
        if hash ~= joaat("WEAPON_UNARMED") then
            if use_generic and generic_hash_string then
                ...
                local weapons_list = {
                    "WEAPON_BAT", "WEAPON_KNIFE", "WEAPON_HAMMER", "WEAPON_CROWBAR", "WEAPON_GOLFCLUB", "WEAPON_BOTTLE",
                    "WEAPON_DAGGER", "WEAPON_HATCHET", "WEAPON_KNUCKLE", "WEAPON_MACHETE", "WEAPON_FLASHLIGHT",
                    ...
                    "WEAPON_REVOLVER_MK2", "WEAPON_DOUBLEACTION", "WEAPON_RAYPISTOL", "WEAPON_CERAMICPISTOL",
                    ...
                    "WEAPON_SNOWBALL", "WEAPON_PIPEBOMB", "WEAPON_FLARE",
                    "WEAPON_PARACHUTE", "WEAPON_FIREEXTINGUISHER", "WEAPON_PETROLCAN", "WEAPON_HAZARDCAN"
                }
    ...
    gui.add_option("💥 Hollow Point Rounds", "Extra damage to unarmored",
        function() apply_comp("_CLIP_HOLLOWPOINT", false) end)
    ...
    gui.add_option("🔇 Suppressor (Pistols)", "Silences weapon",
        function() apply_comp("", true, "COMPONENT_AT_PI_SUPP") end)
    gui.add_option("🔇 Suppressor (Rifles)", "Silences weapon",
        function() apply_comp("", true, "COMPONENT_AT_AR_SUPP") end)
    gui.add_option("🔇 Suppressor (Snipers)", "Silences weapon",
        function() apply_comp("", true, "COMPONENT_AT_SR_SUPP") end)
    gui.add_option("🔦 Flashlight (Pistols)", "Tactical light",
        function() apply_comp("", true, "COMPONENT_AT_PI_FLSH") end)
    ...
    gui.add_option("🔭 Night Vision Scope", "Advanced NV optic",
        function() apply_comp("", true, "COMPONENT_AT_SCOPE_NV") end)
    gui.add_option("🔭 Thermal Scope", "Advanced thermal optic",
        function() apply_comp("", true, "COMPONENT_AT_SCOPE_THERMAL") end)
    gui.set_submenu_context(ammunation_menu)
    ...
    gui.add_option("🖌️ Apply Tint", "Applies the selected tint to current weapon", apply_tint)
    gui.add_break("=== MK II FINISHES ===")
    gui.add_option("✨ Luxe Finish", "Gold / Platinum", function() apply_comp("_VARMOD_LUXE", false) end)
    gui.set_submenu_context(ammunation_menu)

    gui.set_submenu_context(wep_menu)
    gui.add_bool_option("♾️ Infinite Ammo", "Never reload again", state.infinite_ammo)
    ...
    gui.add_option("🔫 Give All Weapons", "Grant standard weapon loadout", give_all_weapons)
    gui.add_option("⭐ Max All Weapons", "Fully upgrades all your weapons", max_all_weapons)
    gui.add_option("🗑️ Drop Current Weapon", "Drop equipped weapon", drop_current_weapon)

    gui.add_break("=== QUICK ACTIONS ===")
    gui.add_option("👽 Give Up-n-Atomizer", "Spawns the raygun", give_up_n_atomizer)
    gui.add_option("🚫 Remove All Weapons", "Strips your weapon wheel", remove_all_weapons)
    gui.reset_submenu_context()
    ...
    gui.add_bool_option("🖥️ On-Screen HUD", "Draws health and coordinates", state.info_hud)
    gui.add_bool_option("👁️ Player ESP Text", "Draws distances to other players", state.player_esp)
    gui.add_break("=== ENVIRONMENT ===")
    gui.add_bool_option("⏱️ Time Control", "Override session time", state.time_control)
    ...
    gui.add_option("☀️ Extra Sunny", "Set weather", function() MISC.SET_WEATHER_TYPE_NOW_PERSIST("EXTRASUNNY") end)
    gui.add_option("🌧️ Rain", "Set weather", function() MISC.SET_WEATHER_TYPE_NOW_PERSIST("RAIN") end)
    gui.add_option("⛈️ Thunder", "Set weather", function() MISC.SET_WEATHER_TYPE_NOW_PERSIST("THUNDER") end)
    gui.add_option("❄️ Snow", "Set weather", function() MISC.SET_WEATHER_TYPE_NOW_PERSIST("SNOW") end)
    gui.set_submenu_context(world_menu)

    gui.add_option("🧹 Clear Area", "Delete nearby entities", clear_area)
    gui.add_break("=== WORLD ACTIONS ===")
    gui.add_option("🚗 Delete Nearby Vehicles", "Wipes all empty vehicles from the area", delete_nearby_vehicles)
    gui.add_option("😂 Fetch Random Joke (HTTP)", "Tests the HTTP module", fetch_random_joke_http)
    gui.reset_submenu_context()
...
local function teleport_to_waypoint()
    ...
    gui.add_break("=== ⚙️ SETTINGS ===")
    gui.add_bool_option("🔄 Smooth Transitions", "Use story mode character switch transition", state.smooth_transitions)
    gui.add_break("=== 📍 QUICK TELEPORT ===")
    gui.add_option("🚩 Teleport to Waypoint", "Teleport to your placed waypoint", teleport_to_waypoint)
    gui.add_break("=== 🌆 LOS SANTOS ===")
    gui.add_option("🔧 Los Santos Customs", "Mod shop",
        function()
            smart_teleport(-360.0, -130.0, 38.0)
            notify.success("📍 LS Customs")
        end)
    gui.add_option("🔫 Ammu-Nation", "Gun store",
        function()
            smart_teleport(22.0, -1105.0, 29.0)
            notify.success("📍 Ammu-Nation")
        end)
    gui.add_option("🏢 Maze Bank Tower", "Tallest building",
        function()
            smart_teleport(-75.0, -820.0, 326.0)
            notify.success("📍 Maze Bank")
        end)
    gui.add_option("🎬 Vinewood Sign", "Hollywood sign",
        function()
            smart_teleport(730.0, 1080.0, 360.0)
            notify.success("📍 Vinewood Sign")
        end)
    gui.add_option("🎡 Del Perro Pier", "Pier",
        function()
            smart_teleport(-1700.0, -1050.0, 13.0)
            notify.success("📍 Del Perro Pier")
        end)
    gui.add_option("🏖️ Beach", "Vespucci Beach",
        function()
            smart_teleport(-1200.0, -1450.0, 4.0)
            notify.success("📍 Beach")
        end)
    gui.add_option("✈️ LSIA", "Airport", function()
        smart_teleport(-1300.0, -2900.0, 13.0)
        notify.success("📍 LSIA")
    end)
    gui.add_break("=== 🎵 NIGHTCLUBS & BARS ===")
    gui.add_option("🎸 Tequi-La-La", "Rock bar entrance",
        function()
            smart_teleport(-560.0, 285.0, 71.0)
            notify.success("📍 Tequi-La-La")
        end)
    gui.add_option("🪩 Bahama Mamas", "Nightclub",
        function()
            smart_teleport(-1388.0, -620.0, 30.0)
            notify.success("📍 Bahama Mamas")
        end)
    gui.add_option("🎧 The Music Locker", "Casino nightclub",
        function()
            smart_teleport(925.0, 55.0, 71.0)
            notify.success("📍 Music Locker")
        end)
    gui.add_option("💃 Vanilla Unicorn", "Strip club",
        function()
            smart_teleport(130.0, -1300.0, 29.0)
            notify.success("📍 Vanilla Unicorn")
        end)
    gui.add_option("👠 Strip Club (Inside)", "Inside Vanilla Unicorn",
        function()
            smart_teleport(108.0, -1305.0, 28.7)
            notify.success("📍 Inside Strip Club")
        end)
    gui.add_break("=== 🎰 CASINO ===")
    gui.add_option("🏛️ Diamond Casino Entrance", "Main entrance",
        function()
            smart_teleport(935.0, 45.0, 81.0)
            notify.success("📍 Casino")
        end)
    gui.add_option("🏛️ Diamond Casino Roof", "Rooftop",
        function()
            smart_teleport(920.0, 55.0, 120.0)
            notify.success("📍 Casino Roof")
        end)
    gui.add_break("=== 🏜️ BLAINE COUNTY ===")
    gui.add_option("🏠 Sandy Shores", "Desert town",
        function()
            smart_teleport(1800.0, 3700.0, 32.0)
            notify.success("📍 Sandy Shores")
        end)
    gui.add_option("🏠 Paleto Bay", "North coast town",
        function()
            smart_teleport(-450.0, 6020.0, 31.0)
            notify.success("📍 Paleto Bay")
        end)
    gui.add_option("🏰 Fort Zancudo", "Military base",
        function()
            smart_teleport(-2200.0, 3200.0, 32.0)
            notify.success("📍 Fort Zancudo")
        end)
    gui.add_break("=== ⛰️ MOUNTAINS ===")
    gui.add_option("🏔️ Mount Chiliad Peak", "Highest point",
        function()
            smart_teleport(450.0, 5600.0, 800.0)
            notify.success("📍 Mount Chiliad")
        end)
    gui.add_break("=== 🌊 DEEP WATER LOCATIONS ===")
    gui.add_option("🌊 Deep Ocean Trench", "Dive deep underwater (-40m)",
        function()
            smart_teleport(-500.0, 6500.0, -40.0)
            notify.success("📍 Ocean Trench (deep dive)")
        end)
    gui.add_option("🚢 Sunken Shipwreck", "Dive to the shipwreck (-40m)",
        function()
            smart_teleport(-600.0, 6800.0, -40.0)
            notify.success("📍 Shipwreck (deep dive)")
        end)
    gui.add_option("🌊 North Coast", "North shore underwater (-40m)",
        function()
            smart_teleport(500.0, 7000.0, -40.0)
            notify.success("📍 North Coast (deep dive)")
        end)
    gui.reset_submenu_context()

    -- Submenu: Protections
    local protections_menu = gui.add_submenu("🛡️ Protections", "Defend against other modders")
    gui.set_submenu_context(protections_menu)
    gui.add_bool_option("🛡️ Block Malicious Script Events", "Intercepts bad TSEs", state.block_script_events)
    ...
    gui.add_bool_option("🛡️ Projectile Shield", "Deletes incoming rockets and grenades", state.projectile_shield)
    gui.add_bool_option("🗑️ Auto-Delete Cages", "Instantly destroys objects spawned on you", state.auto_delete_cages)
    gui.add_option("🔓 Break Free", "Instantly escape frozen states and clear area", break_free)
    gui.reset_submenu_context()
    ...
    local settings_menu = gui.add_submenu("⚙️ Settings", "Menu configuration and credits")
    gui.set_submenu_context(settings_menu)

    gui.add_break("=== SCRIPT DATA ===")
    gui.add_option("💾 Save Config", "Saves all current toggle states", save_config)
    gui.add_option("🔄 Reload Config", "Reloads toggles from disk", reload_config)
    ...
    gui.add_option("👑 @iitztoasty/Toastizle", "Creator and Lead Developer",
        function() notify.info("Thank you for using YNAAIOMM!") end)
